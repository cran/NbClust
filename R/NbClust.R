...
NbClust <- function(data, diss="NULL", distance = "euclidean", min.nc=2, max.nc=15, method = "ward", index = "all", alphaBeale = 0.1)
{
 ...
 if (((distance==1) || (distance==2) || (distance==3) || (distance==4) || (distance==5) || (distance==6)) && (diss[1] != "NULL")) {
  ...
  method <- pmatch(method, c("ward", "single", "complete", "average", 
    ...
    res <- array(0, c(max_nc-min_nc+1,31))
    res[,1] <- min_nc:max_nc
    rownames(res) <- min_nc:max_nc
    resCritical <- array(0, c(max_nc-min_nc+1,5))
    resCritical[,1] <- min_nc:max_nc
    rownames(resCritical) <- min_nc:max_nc
    colnames(resCritical) <- c("nc.CritValue","CritValue_Duda", "CritValue_PseudoT2", "Fvalue_Beale", "CritValue_Gap")
    ...
    if (method == 1) 
    {
        hc<-hclust(md,method = "ward")
      	colnames(res) <- c("nc.Ward","index.KL","index.CH","index.Hartigan","index.CCC","index.Scott","index.Marriot",
			"index.TrCovW", "index.TraceW","index.Friedman","index.Rubin","index.Cindex","index.DB",
			"index.Silhouette", "index.Duda", "index.Pseudot2", "index.Beale", "index.Ratkowsky",
			"index.Ball", "index.ptbiserial", "index.Gap", "index.Frey", "index.McClain","index.Gamma",
			"index.Gplus", "index.Tau", "index.Dunn", "index.Hubert", "index.SDindex", "index.Dindex", "index.SDbw")
   }
    if (method == 2) 
    {
...
#####################################################################
### Indices duda, pseudot2 and beale
#####################################################################    
...
#####################################################################
### Indices Kl, Ch, Hartigan, Ratkowsky and Ball
#####################################################################
...
#################################################################################################################################
 ...
 {  
  ...
  ########### Indices.Traces-hartigan - 4e colonne de res ############ 
 	if (any(indice == 3) || (indice == 31) || (indice == 32))
	{    
	  res[nc-min_nc+1,4] <- Indices.Traces(jeu, md, clall1, index = "hart")	
	} 
  ...
  ########### Indices.WKWL-beale - 17e colonne de res ############
	...
	{             	  
	  res[nc-min_nc+1,17] <- beale <- Indices.WKWL(x=jeu,cl1=cl1,cl2=cl2)$beale
	}
  ...
  if (any(indice == 14) || (indice == 15) || (indice == 16) || (indice == 31) || (indice == 32))
	{      	  
	  ...
	  {
	  	df2 <- (NM-2)*pp
	  	resCritical[nc-min_nc+1,4] <- 1-pf(beale,pp,df2)
	  }
	}

  ########### Indices.TracesL-ball - 19e colonne de res ############
	if (any(indice == 18) || (indice == 31) || (indice == 32))
	{        	  
	  res[nc-min_nc+1,19] <- Indices.Traces(jeu, md, clall1, index = "ball")
	}

  ########### Indice.Point-Biserial - 20e colonne de res ############ 
	if (any(indice == 19) || (indice == 31) || (indice == 32))
	{       
	  res[nc-min_nc+1,20] <- Indice.ptbiserial(x=jeu, md=md, cl1=cl1)     
	}
   
  ########### Gap - 21e colonne de res ############       	
	if (any(indice == 20) || (indice == 32))
	{
	  ...
	  if (method == 8) {
    	...
    	res[nc-min_nc+1,21] <- resultSGAP$gap
	resCritical[nc-min_nc+1,5] <- resultSGAP$diffu
	}

	if (nc >=2)
	{
	   ########### Indices.Traces-kl - 2e colonne de res ############ 	  
 	   if (any(indice == 1) || (indice == 31) || (indice == 32)) 
	   {	 
        res[nc-min_nc+1,2] <- Indices.Traces(jeu, md, clall1, index = "kl")
	   }

     ########### Indices.Traces-ch - 3e colonne de res ############
 	   if (any(indice == 2) || (indice == 31) || (indice == 32)) 
	   {		   
	      res[nc-min_nc+1,3] <- Indices.Traces(jeu, md, clall1, index = "ch")
	   }
	   
	   ########### Indice.cindex - 12e colonne de res ############
	   if (any(indice == 11) || (indice == 31) || (indice == 32)) 
	   {	  	   
	      res[nc-min_nc+1,12] <- Indice.cindex(d=md, cl=cl1)
	   }

     ########### Indice.DB  - 13e colonne de res ############
 	   if (any(indice == 12) || (indice == 31) || (indice == 32)) 
	   {		   
	      res[nc-min_nc+1,13] <- Indice.DB(x=jeu, cl=cl1, d = NULL, centrotypes = "centroids", p = 2, q = 2)$DB
	   }                         

     ########### Silhouette - 14e colonne de res ############
 	   if (any(indice == 13) || (indice == 31) || (indice == 32)) 
	   {		   
	      res[nc-min_nc+1,14] <- Indice.S(d=md, cl=cl1)
	   }
	   
	   ########### Indices.Traces-ratkowsky- 18e colonne de res ############
	   	if (any(indice == 17) || (indice == 31) || (indice == 32))
	   {  	  
	      res[nc-min_nc+1,18] <- Indices.Traces(jeu, md, clall1, index = "ratkowsky")
     }
     ...
     ########### Indice.McClain - 23e colonne de res ############
	   ...
	   {  	     
	     res[nc-min_nc+1,23] <- Index.15and28(cl1=cl1,cl2=cl2,md=md)$mcclain
	   }
     ...
     ########### Indice.Gplus- 25e colonne de res ############
	   ...
	   {    	     
	     res[nc-min_nc+1,25] <- Index.sPlussMoins(cl1=cl1,md=md)$gplus
	   }

     ########### Indice.Tau  - 26e colonne de res ############
	   if (any(indice == 25) || (indice == 32))
	   {   	     
	     res[nc-min_nc+1,26] <- Index.sPlussMoins(cl1=cl1,md=md)$tau
	   } 
     ...
     if (any(indice == 27 ) || (indice == 31) || (indice == 32))
	   {         	     
	     res[nc-min_nc+1,28] <- Index.Hubert(jeu, cl1)	
	   }	 
	   
	   ########### Indices.SD - 29e colonne de res ############
     if (any(indice == 28 ) || (indice == 31) || (indice == 32))
	   {	    
	    res[nc-min_nc+1,29] <- Index.sdindex(jeu, clmax, cl1)
	   }	
	   
	   ########### Indices.Dindex - 30e colonne de res ############ 
	   	if (any(indice == 29 ) || (indice == 31) || (indice == 32))
	    {        	        
	        res[nc-min_nc+1,30] <- Index.Dindex(cl1, jeu)	
      }  
	   
	    ########### Indices.SDbw - 31e colonne de res ############
	   if (any(indice == 30 ) || (indice == 31) || (indice == 32))
	   { 	      
	       res[nc-min_nc+1,31] <- Index.SDbw(jeu, cl1)	
	   }      	
	...
	else
  {
	...
	res[nc-min_nc+1,3] <- NA
	res[nc-min_nc+1,12] <- NA
	res[nc-min_nc+1,13] <- NA
	res[nc-min_nc+1,14] <- NA
	res[nc-min_nc+1,18] <- NA
	res[nc-min_nc+1,22] <- NA
...
#########################################################################################################
   ...
   if (any(indice == 1) || (indice == 31) || (indice == 32)) 
     ...
     nc.KL <- (min_nc:max_nc)[which.max(res[,2])]
     indice.KL <- max(res[,2],na.rm = TRUE)
   }
   ...
   if (any(indice == 2) || (indice == 31) || (indice == 32)) 
     ...
     nc.CH <- (min_nc:max_nc)[which.max(res[,3])]
     indice.CH <- max(res[,3],na.rm = TRUE)
     #print(paste("The maximum value of index.CH is used to indicate the best number of clusters"))
   }
  ...
  if (any(indice == 4) || (indice == 31) || (indice == 32))
    ...
    #print(paste("The maximum value accross the hierarchy levels is used to indicate the optimal number of clusters")    
   }
  ...
  if (any(indice == 12) || (indice == 31) || (indice == 32)) 
    ...
    nc.DB <- (min_nc:max_nc)[which.min(res[,13])]
    indice.DB <- min(res[,13],na.rm = TRUE)
  }
  ...
  if (any(indice == 13) || (indice == 31) || (indice == 32)) 
    ...
    nc.Silhouette <- (min_nc:max_nc)[which.max(res[,14])]
    indice.Silhouette <- max(res[,14],na.rm = TRUE)
  }

  nc.Gap<-indice.Gap<-0
  # GAP - Choose the number of clusters via finding the smallest q such that: Gap(q)=Gap(q+1)-Sq+1 (q=1,…,n-2) [ClusterSim package].
  if (any(indice == 20) || (indice == 32))
  {
    found <- FALSE
    for (ncG in min_nc:max_nc){
      if ((resCritical[ncG-min_nc+1,5] >=0) && (!found)){
          ncGap <- ncG
     	  indiceGap <- res[ncG-min_nc+1,21]
    	  found <- TRUE
    	  }
     }
     if (found){
  ...
  if (any(indice == 19) || (indice == 31) || (indice == 32))
    ...
    nc.ptbiserial <- (min_nc:max_nc)[which.max(res[,20])]
    indice.ptbiserial <- max(res[,20],na.rm = TRUE)
  }
  ...
  #			      If the ration never fell below 1.00, a one cluster solution was assumed [29].
     ...
     for (ncF in min_nc:max_nc)
     {
        ...
        {
          #print(paste("res[ncF-min_nc+1,22]",res[ncF-min_nc+1,22]))
          
          if (res[ncF-min_nc+1,22] < 1) 
          {
            if(ncF==min_nc)
            {
            #print(paste("pb1"))
            ...
            { 
              ncFrey <- ncF-1               
     	        indiceFrey <- res[ncF-1-min_nc+1,22]
     	        #print(paste("ncF =", ncF ))
              #print(paste("indiceFrey =", indiceFrey ))
  	          foundFrey <- TRUE
              foundNC[i]<-ncFrey
              foundIndice[i]<-indiceFrey
              i<-i+1
  	          #print(paste("foundFrey =", foundFrey ))
              #print(paste("foundNC =", foundNC[i] ))
              #print(paste("foundIndice[i] =", foundIndice[i] ))
  	          #print(paste("pb2"))
              #ncF<-max_nc+1
              #print(paste("ncF=",ncF))
  	        } 
     ...
     if (foundFrey)
     {
  	   ...
  	   #print(paste("foundNC =", foundNC ))
  	   #print(paste("foundIndice =", foundIndice ))
      }
      else 
      {
  	    nc.Frey <- NA
  ...
  nc.McClain <- (min_nc:max_nc)[which.min(res[,23])]
  indice.McClain <- min(res[,23],na.rm = TRUE)
  ...
  nc.Gplus  <- (min_nc:max_nc)[which.min(res[,25])]
  indice.Gplus <- min(res[,25],na.rm = TRUE)
  }
  ...
  nc.Tau <- (min_nc:max_nc)[which.max(res[,26])]
  indice.Tau <- max(res[,26],na.rm = TRUE)
  }
...
#Some indices need to compute difference between hierarchy levels to identify relevant number of clusters
  ...
  if((indice==3)||(indice == 5)||(indice == 6)||(indice == 7)||(indice == 8)||(indice == 9)||(indice == 10)||(indice == 18)||(indice == 27)||(indice == 11)||(indice == 29)||(indice == 31)||(indice == 32))
  {
    ...
    DiffLev[,1] <- min_nc:max_nc
       for (nc3 in min_nc:max_nc)      
      {
        if (nc3==min_nc)
        {    
	       DiffLev[nc3-min_nc+1,2] <- abs(res[nc3-min_nc+1,4]-NA)   # Hartigan
	       DiffLev[nc3-min_nc+1,3] <- abs(res[nc3-min_nc+1,6]-NA)   #Scott
	       DiffLev[nc3-min_nc+1,4] <- abs(res[nc3-min_nc+1,7]-NA)   # Marriot
	       DiffLev[nc3-min_nc+1,5] <- abs(res[nc3-min_nc+1,8]-NA)   #Trcovw
	       DiffLev[nc3-min_nc+1,6] <- abs(res[nc3-min_nc+1,9]-NA)   #Tracew
	       DiffLev[nc3-min_nc+1,7] <- abs(res[nc3-min_nc+1,10]-NA)  #Friedman
	       DiffLev[nc3-min_nc+1,8] <- abs(res[nc3-min_nc+1,11]-NA)  #Rubin
	       DiffLev[nc3-min_nc+1,9] <- abs(res[nc3-min_nc+1,19]-NA)  # Ball
         DiffLev[nc3-min_nc+1,10] <- abs(res[nc3-min_nc+1,28]-NA) # Hubert   
         DiffLev[nc3-min_nc+1,12] <- abs(res[nc3-min_nc+1,30]-NA) # D index
        ...
        {  
          if(nc3==max_nc)
          { 
            DiffLev[nc3-min_nc+1,2] <- abs(res[nc3-min_nc+1,4]-res[nc3-min_nc,4])
            DiffLev[nc3-min_nc+1,3] <- abs(res[nc3-min_nc+1,6]-res[nc3-min_nc,6])
            DiffLev[nc3-min_nc+1,4] <- abs(res[nc3-min_nc+1,7]-NA) # Marriot
            DiffLev[nc3-min_nc+1,5] <- abs(res[nc3-min_nc+1,8]-res[nc3-min_nc,8])  # trcovw
            DiffLev[nc3-min_nc+1,6] <- abs(res[nc3-min_nc+1,9]-NA) #traceW            
            DiffLev[nc3-min_nc+1,7] <- abs(res[nc3-min_nc+1,10]-res[nc3-min_nc,10])
            DiffLev[nc3-min_nc+1,8] <- abs(res[nc3-min_nc+1,11]-NA) #Rubin
            DiffLev[nc3-min_nc+1,9] <- abs(res[nc3-min_nc+1,19]-res[nc3-min_nc,19])
            DiffLev[nc3-min_nc+1,10] <- abs(res[nc3-min_nc+1,28]-NA)
            DiffLev[nc3-min_nc+1,12] <- abs(res[nc3-min_nc+1,30]-NA) # D index  
          ...
          else      
          {
           ...
           DiffLev[nc3-min_nc+1,2] <- abs(res[nc3-min_nc+1,4]-res[nc3-min_nc,4]) # Hartigan              
	         DiffLev[nc3-min_nc+1,3] <- abs(res[nc3-min_nc+1,6]-res[nc3-min_nc,6]) 
           DiffLev[nc3-min_nc+1,4] <- ((res[nc3-min_nc+2,7]-res[nc3-min_nc+1,7])-(res[nc3-min_nc+1,7]-res[nc3-min_nc,7]))
           DiffLev[nc3-min_nc+1,5] <- abs(res[nc3-min_nc+1,8]-res[nc3-min_nc,8])
           DiffLev[nc3-min_nc+1,6] <- ((res[nc3-min_nc+2,9]-res[nc3-min_nc+1,9])-(res[nc3-min_nc+1,9]-res[nc3-min_nc,9]))
           DiffLev[nc3-min_nc+1,7] <- abs(res[nc3-min_nc+1,10]-res[nc3-min_nc,10])
           DiffLev[nc3-min_nc+1,8] <- ((res[nc3-min_nc+2,11]-res[nc3-min_nc+1,11])-(res[nc3-min_nc+1,11]-res[nc3-min_nc,11]))
           DiffLev[nc3-min_nc+1,9] <- abs(res[nc3-min_nc+1,19]-res[nc3-min_nc,19])  
           DiffLev[nc3-min_nc+1,10] <- abs((res[nc3-min_nc+1,28]-res[nc3-min_nc,28]))             
           DiffLev[nc3-min_nc+1,12] <-((res[nc3-min_nc+2,30]-res[nc3-min_nc+1,30])-(res[nc3-min_nc+1,30]-res[nc3-min_nc,30])) #Dindex     
  ...
  # RATKOWSKY - The optimal number of groups is taken as the level where this criterion exhibits its maximum value [29].
    nc.Ratkowsky <- (min_nc:max_nc)[which.max(res[,18])]
    indice.Ratkowsky <- max(res[,18],na.rm = TRUE)
  }
    ...
    if (any(indice == 11) || (indice == 31) || (indice == 32)) 
    {
      ...
      nc.cindex <- (min_nc:max_nc)[which.min(res[,12])]
      indice.cindex <- min(res[,12],na.rm = TRUE)
    }  
  ...
  if (any(indice == 18) || (indice == 31) || (indice == 32))
   ...
   if (any(indice == 26) || (indice == 31) || (indice == 32)) 
	 {
     # Dunn - 
     nc.Dunn <- (min_nc:max_nc)[which.max(res[,27])]
     indice.Dunn <- max(res[,27],na.rm = TRUE)
   }
   ...
   if (any(indice == 28) || (indice == 31) || (indice == 32)) 
	 {
     # SD - 
     nc.sdindex <- (min_nc:max_nc)[which.min(res[,29])]
     indice.sdindex<- min(res[,29],na.rm = TRUE)
   }
    ...
    if (any(indice == 30) || (indice == 31) || (indice == 32)) 
	   {
       # SDbw - 
       nc.SDbw <- (min_nc:max_nc)[which.min(res[,31])]
       indice.SDbw<- min(res[,31],na.rm = TRUE)     
     } 
...
######################################################################################################################
 ...
 if (any(indice == 20) || (indice == 23) || (indice == 24) || (indice == 25) || (indice == 32))
 {
  ...
  resultats <- matrix(c(results),nrow=2,ncol=30,dimnames = list(c("Number_clusters","Value_Index"),
		     c("index.KL","index.CH","index.Hartigan","index.CCC", "index.Scott", "index.Marriot", "index.TrCovW",
		       "index.TraceW", "index.Friedman", "index.Rubin", "index.Cindex", "index.DB", "index.Silhouette",
			"index.Duda","index.PseudoT2", "index.Beale", "index.Ratkowsky", "index.Ball", "index.PtBiserial",
			"index.Gap", "index.Frey", "index.McClain", "index.Gamma", "index.Gplus", "index.Tau", "index.Dunn", 
 ...
 else
  {
    ...
    resultats <- matrix(c(results),nrow=2,ncol=26,dimnames = list(c("Number_clusters","Value_Index"),
		     c("index.KL","index.CH","index.Hartigan","index.CCC", "index.Scott", "index.Marriot", "index.TrCovW",
		       "index.TraceW", "index.Friedman", "index.Rubin", "index.Cindex", "index.DB", "index.Silhouette",
			"index.Duda","index.PseudoT2", "index.Beale", "index.Ratkowsky", "index.Ball", "index.PtBiserial",
			"index.Frey", "index.McClain", "index.Dunn", "index.Hubert", "index.SDindex", "index.Dindex", "index.SDbw")))
   }
 ...
 { 
  ...
  resultats[1,]<-round(resultats[1,], digits=2)  
  resultats<-round(resultats, digits=4)


  if (numberObsAfter != numberObsBefore) 
  {
     print(paste(numberObsAfter,"observations were used out of", numberObsBefore ,"possible observations due to missing values."))
  }
  ...
  if (numberObsAfter == numberObsBefore) 
  {
    ...
    { 
      results.final <- list(All.index=res,All.CriticalValues=resCritical,Best.nc=resultats)
    }
    else
    { 
